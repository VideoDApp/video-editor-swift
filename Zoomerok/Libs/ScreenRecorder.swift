...
final public class ScreenRecorder {
  ...
  */
    public func startRecording(to outputURL: URL? = nil,
        ...
        completeHandler: @escaping (URL?, Error?) -> Void) {
        let outUrl = createVideoWriter(in: outputURL, error: errorHandler)
        ...
        startCapture(error: errorHandler, complete: {result in
            completeHandler(outUrl, result)
        })
    }
    ...
    private func createVideoWriter(in outputURL: URL? = nil, error: (Error) -> Void) -> URL? {
    ...
    private func startCapture(error: @escaping (Error) -> Void, complete: @escaping (Error?) -> Void) {
  ...
  - Parameter errorHandler: Called when an error is found
  */
    public func stoprecording(saveToCameraRoll: Bool, errorHandler: @escaping (Error) -> Void, completeHandler: @escaping (URL?, Error?) -> Void) {
        RPScreenRecorder.shared().stopCapture(handler: { error in
            if let error = error {
                errorHandler(error)
            }

            completeHandler(self.videoOutputURL, error)
        })
